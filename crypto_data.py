        ...
        try:
            ...
            for support_price, period in support_levels:
                if support_price and current_price > support_price:  # Price must be above support
                    ...
                    if 0.01 <= distance_from_support <= threshold:  # Minimum 1% buffer for crypto volatility
                        zones.append(period)
            ...
            volume_24h = int(hist["Volume"].iloc[-1]) if not hist["Volume"].empty else None
            market_cap = None  # Placeholder, as direct market cap isn't available in yfinance
        ...
        try:
            ...
            for resistance_price, period in resistance_levels:
                if resistance_price and current_price < resistance_price:  # Price must be below resistance
                    ...
                    # Be more conservative: price should be close but not too close (risk of breaking)
                    if 0.01 <= distance_from_resistance <= threshold:  # Minimum 1% buffer for crypto volatility
                        zones.append(period)
